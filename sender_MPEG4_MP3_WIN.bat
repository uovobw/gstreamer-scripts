...
gst-launch-0.10.exe -v gstrtpbin name=rtpbin ^
    dshowvideosrc ! video/x-raw-yuv,width=(int)320,height=(int)240,framerate=(fraction)30/1 ! ffmpegcolorspace ! tee name="t" ! ^
    videorate ! videoscale ! ffenc_mpeg4 max-key-interval=1 max-bframes=2 ! rtpmp4vpay send-config=true ! rtpbin.send_rtp_sink_0 ^
    ...
    t. ! autovideosink sync=true async=true
