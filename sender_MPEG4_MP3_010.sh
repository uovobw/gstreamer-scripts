...
gst-launch-0.10 -v gstrtpbin name=rtpbin \
    v4l2src ! video/x-raw-yuv,width=\(int\)320,height=\(int\)240,framerate=\(fraction\)30/1 ! tee name="t" ! ffmpegcolorspace ! \
    videoscale ! videorate ! ffenc_mpeg4 max-key-interval=1 max-bframes=2 ! rtpmp4vpay send-config=true ! rtpbin.send_rtp_sink_0 \
